/**
 * Utility block definitions - helper blocks for signal conversion and manipulation
 */
 ...
 * Utility block types
 */
export type UtilityBlockType = "note-to-freq" | "note-to-freq-poly";
...
export function getNoteFrequency(note: string, octave: number): number {
  const frequencies = NOTE_FREQUENCIES[note];
  if (!frequencies || octave < 0 || octave > 8) {
    return 0;
  }
...
/**
 * Utility block definitions
 */
export const UTILITY_DEFINITIONS: Record<UtilityBlockType, BlockDefinition> = {
  "note-to-freq": {
    type: "note-to-freq",
    label: "Note to Freq",
    inputs: [
    ...
    outputs: [{ id: "freq", label: "Freq" }],
    defaultConfig: {
      noteToFreqOctave: 4, // Middle octave (A4 = 440 Hz)
    },
  },
  "note-to-freq-poly": {
    type: "note-to-freq-poly",
    label: "Note to Freq (Poly)",
    inputs: [
      { id: "C", label: "C" },
      { id: "C#", label: "C#" },
      { id: "D", label: "D" },
