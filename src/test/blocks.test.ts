...
describe('Block Definitions', () => {
  ...
  it('should have valid default configurations', () => {
    Object.entries(BLOCK_DEFINITIONS).forEach(([type, definition]) => {
      ...
      if (['sine-wave', 'square-wave', 'triangle-wave', 'sawtooth-wave'].includes(type)) {
      ...
      if (['low-pass-filter', 'high-pass-filter', 'band-pass-filter'].includes(type)) {
...
describe('getBlockInputs', () => {
  ...
  it('should return correct inputs for FFT analyzer based on mode', () => {
    ...
    const spectrumInputs = getBlockInputs('fft-analyzer', { fftMode: 'spectrum' });
    expect(spectrumInputs).toHaveLength(1);

    // Frequency output mode - single input
    const freqInputs = getBlockInputs('fft-analyzer', { fftMode: 'frequency-output' });
    expect(freqInputs).toHaveLength(1);

    // Peak detection mode - single input
    const peakInputs = getBlockInputs('fft-analyzer', { fftMode: 'peak-detection' });
    expect(peakInputs).toHaveLength(1);

    // Spectral processing mode - single input
    const spectralInputs = getBlockInputs('fft-analyzer', { fftMode: 'spectral-processing' });
...
describe('getBlockOutputs', () => {
  ...
  it('should return correct outputs for math blocks', () => {
    ...
    const mathBlocks: BlockType[] = [
      ...
      'ceil', 'floor', 'round', 'abs', 'sign', 'negate',
      'sqrt', 'sin', 'cos', 'min', 'max', 'pow', 'mod', 'clamp'
    ];
  ...
  it('should return correct outputs for FFT analyzer based on mode', () => {
    ...
    const spectrumOutputs = getBlockOutputs('fft-analyzer', { fftMode: 'spectrum' });
    ...
    const peakOutputs = getBlockOutputs('fft-analyzer', { fftMode: 'peak-detection' });
    expect(peakOutputs).toHaveLength(0);

    // Spectral processing mode - single output
    const spectralOutputs = getBlockOutputs('fft-analyzer', { fftMode: 'spectral-processing' });
